...
void InstrumentConfig::save() {
    ...
    for (const auto& pair : instruments) {
        const InstrumentInfo& info = pair.second;
...
int InstrumentConfig::find_or_create_instrument(const std::array<uint8_t, 32>& waveform_data, const std::string& source_filename) {
    ...
    if (instruments.count(fp)) {
        return instruments[fp].midi_instrument;
    }

    for (const auto& pair : instruments) {
        const auto& existing_info = pair.second;
        std::array<uint8_t, 32> existing_wave;
        for(size_t i = 0; i < 32; ++i) {
            std::string byte_str = existing_info.fingerprint.substr(i * 2, 2);
            existing_wave[i] = static_cast<uint8_t>(std::stoul(byte_str, nullptr, 16));
        }
        if (are_waveforms_similar(waveform_data, existing_wave, 6)) {
            return existing_info.midi_instrument;
        }
    }
