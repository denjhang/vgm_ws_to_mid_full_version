...
void process_vgm_data(WonderSwanChip& chip, const std::vector<uint8_t>& data, uint32_t& current_pos, uint32_t end_pos) {
    while (current_pos < end_pos) {
        uint8_t command_byte = data[current_pos];

        switch (command_byte) {
            case 0x61: { // Wait nnnn samples
                if (current_pos + 2 >= end_pos) { current_pos = end_pos; return; }
            ...
            case 0x67: { // data block
                if (current_pos + 6 >= end_pos) { current_pos = end_pos; return; }
...
int main(int argc, char* argv[]) {
    ...
    bool loop_point_reached = false;

    // Process up to the loop point (or end of file if no loop)
    uint32_t first_pass_end = (loop_offset != 0) ? loop_offset : data.size();
    process_vgm_data(chip, data, current_pos, first_pass_end);

    // If there's a loop, process the loop section once
    if (loop_offset != 0) {
        const size_t channel_count = chip.get_channel_count();
        std::vector<uint32_t> loop_start_ticks(channel_count);
        for (size_t i = 0; i < channel_count; ++i) {
            loop_start_ticks[i] = midi_writer.get_track(i).get_current_time();
        }

        process_vgm_data(chip, data, current_pos, data.size());

        std::vector<uint32_t> loop_end_ticks(channel_count);
        for (size_t i = 0; i < channel_count; ++i) {
            loop_end_ticks[i] = midi_writer.get_track(i).get_current_time();
        }

        // Copy the looped section once (for a total of 2 plays)
        std::cout << "Loop detected. Duplicating looped MIDI events once." << std::endl;
        for (size_t i = 0; i < channel_count; ++i) {
            MidiTrack& target_track = midi_writer.get_track(i);
            target_track.copy_events_from(target_track, loop_start_ticks[i], loop_end_ticks[i]);
        }
    }
