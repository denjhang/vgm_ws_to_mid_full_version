...
void convert_file(const std::string& input_filename, const std::string& output_filename, InstrumentConfig& config, UsageLogger& logger) {
    ...
    const int num_loops = 0; // No loops for batch conversion
    int loops_done = 0;
...
int main(int argc, char* argv[]) {
    ...
    std::string config_filename = "vgm_ws_to_mid/instruments.ini";
    std::string log_filename = "vgm_ws_to_mid/conversion_log.txt";

    UsageLogger logger(log_filename);
    InstrumentConfig config(config_filename, logger);
    config.load();
    
    convert_file(input_filename, output_filename, config, logger);
